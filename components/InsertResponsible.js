...
export default class InsertResponsible extends React.Component {
  ...
  render() {
    ...
    return (
      <KeyboardAvoidingView behavior={Platform.OS == "ios" ? "padding" : "height"} style={styles.wrapper}>
        <View style={styles.container}>
          <Text style={styles.title}>Adding the responsible</Text>
          <TextInput
          ...
          style={styles.textInput}
          onChangeText={this._handleNumber('iin')}
  ...
  _handleinsert = async () => {
    if (this.state.iin == '' || this.state.name == '') {
      alert("Write all fields");
      return;
    }
    if(this.state.iin.size != 12) {
      alert("Please, check iin again");
      return ;
    }
    // Validation of input
    let val = await deviceStorage.retrieveItem('access_token');
    fetch(GLOBALS.BASE_URL + '/zavhoz/insertResponsible.php', {
      method: 'POST',
      headers: {
      ...
      body: 'name=' + this.state.name + '&iin=' + this.state.iin,
    })
